...
- [ ] Task: Implement Pydantic schemas `SummarizeRequest` and `Summary` in `fastapi-backend/app/db/schemas.py`. (AC: 1)
- [ ] Task: Create `fastapi-backend/app/llm_integrations/summarizer.py` for LLM prompt construction and API interaction. (AC: 3)
- [ ] Task: Create `fastapi-backend/app/services/summarization_service.py` to fetch content, call the summarizer, and handle different difficulty levels. (AC: 1, 3)
- [ ] Task: Implement FastAPI endpoint `POST /api/summarize` in `fastapi-backend/app/api/v1/summarize.py` that utilizes the summarization service. (AC: 1, 2)
- [ ] Task: Implement robust error handling in `summarization_service.py` and `summarizer.py` for LLM API failures. (AC: 2)
- [ ] Task: Write Integration tests for `POST /api/summarize` endpoint using `pytest` and `httpx`, ensuring correct summarization logic and error handling (mocking LLM calls). (AC: 1, 2, 3)
- [ ] Task: Update `fastapi-backend/app/main.py` to include the new summarization router.
